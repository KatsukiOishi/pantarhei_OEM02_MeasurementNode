...
初期電力予算は1サイクル10 mJ以下、10分平均0.15 mW以下を受入上限とする。`VCAP >= 4.00 V` は初号機の保守値であり、最大送信条件の電圧降下を確認後、無風時の通信継続を延ばすため3.7 V付近まで下げることを検討する。根拠は[計測・通信レイヤ 電力収支評価](../Architecture/POWER_BUDGET_ASSESSMENT.md)に記録する。

`VCAP < 4.00 V` を検出した周期では、センサ測定、Advertising、NVS書込みを開始しない。`VCAP` が下降時3.58 Vに達するとBuckがハードウェア停止するため、終了処理が動くことを前提にしない。再充電により上昇時4.09 Vへ達した後はリセット起動となるため、必要な状態は十分な電圧がある時点で不揮発化し、起動時に整合性を検査する。
...
- スーパーキャパシタの自己放電を含む10分平均を測り、0.15 mW以下であることを確認する。
